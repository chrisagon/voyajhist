...
function chapitre_insert(){
	...
	set_record_owner('chapitre', $recID, getLoggedMemberID());
...
function chapitre_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = ''){
	...
	if($selected_id){
		...
		if( $dvprint) $templateCode = str_replace('<%%VALUE(Mission)%%>', safe_html($urow['Mission']), $templateCode);
		if(!$dvprint) $templateCode = str_replace('<%%VALUE(Mission)%%>', html_attr($row['Mission']), $templateCode);
		...
		if( $dvprint) $templateCode = str_replace('<%%VALUE(rattache_a_dossier)%%>', safe_html($urow['rattache_a_dossier']), $templateCode);
		if(!$dvprint) $templateCode = str_replace('<%%VALUE(rattache_a_dossier)%%>', html_attr($row['rattache_a_dossier']), $templateCode);
	...
	if($selected_id){
		...
		if($jdata === false) $jdata = get_defaults('chapitre');
		$rdata = $row;
	}
	$templateCode .= loadView('chapitre-ajax-cache', array('rdata' => $rdata, 'jdata' => $jdata));
