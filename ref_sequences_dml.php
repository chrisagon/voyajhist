...
function ref_sequences_insert(){
	...
	set_record_owner('ref_sequences', $recID, getLoggedMemberID());
...
function ref_sequences_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = ''){
	...
	// process values
	if($selected_id){
		if( $dvprint) $templateCode = str_replace('<%%VALUE(id_seqref)%%>', safe_html($urow['id_seqref']), $templateCode);
		if(!$dvprint) $templateCode = str_replace('<%%VALUE(id_seqref)%%>', html_attr($row['id_seqref']), $templateCode);
		...
		if( $dvprint) $templateCode = str_replace('<%%VALUE(num_ord)%%>', safe_html($urow['num_ord']), $templateCode);
		if(!$dvprint) $templateCode = str_replace('<%%VALUE(num_ord)%%>', html_attr($row['num_ord']), $templateCode);
	...
	if($selected_id){
		...
		if($jdata === false) $jdata = get_defaults('ref_sequences');
		$rdata = $row;
	}
	$templateCode .= loadView('ref_sequences-ajax-cache', array('rdata' => $rdata, 'jdata' => $jdata));
