<?php
	...
	*/
	...
	function getTableList($skip_authentication = false){
		...
		$arrTables = array(   
			'redacteur' => array('Passeport R&#233;dacteur', 'Voici votre passeport de R&#233;dacteur :<br>Il recense votre identit&#233;, vos cr&#233;ations et vos badges.', 'resources/table_icons/administrator.png', 'Zone d\'assemblage Creative'),
			'Dossier_histoire' => array('Dossier Histoire', 'Cr&#233;er son dossier Histoire : vous rassemblez ici les personnages, les objets et les d&#233;cors dans lequel va se d&#233;rouler votre histoire ou votre jeu.', 'resources/table_icons/chart_organisation.png', 'Zone d\'assemblage Creative'),
			'chapitre' => array('Chapitre', 'Assemblez ici le chapitre de votre histoire', 'resources/table_icons/align_right.png', 'Domaine cr&#233;atif'),
			'sequence' => array('Sequences', 'S&#233;quence de votre sc&#233;nario. Votre histoire est d&#233;coup&#233;e en chapitre, sc&#232;ne, s&#233;quence. C\'est ici que vous d&#233;crivez chacune de vos s&#233;quences.', 'resources/table_icons/attributes_display.png', 'Domaine cr&#233;atif'),
			'Objectifs' => array('Objectifs', 'Objectif du jeu : quel score faut-il atteindre pour gagner votre jeu ?', 'resources/table_icons/compass.png', 'Domaine cr&#233;atif'),
			'moyens' => array('Moyens', 'Moyens &#224; la disposition du joueur pour atteindre le meilleur score', 'resources/table_icons/brick.png', 'Domaine cr&#233;atif'),
			'Objets' => array('Objets', 'r&#233;f&#233;rentiel des objets &#224; utiliser dans le dossier HEXILIS', 'resources/table_icons/color_swatch.png', 'Domaine cr&#233;atif'),
			'personnages' => array('Personnages', 'Les h&#233;ros incarn&#233;s par vos joueurs, les PNJ (personnages non joueurs) amis ou ennemis sont d&#233;crit ici.', 'resources/table_icons/client_account_template.png', 'Domaine cr&#233;atif'),
			'lieu_et_scene' => array('Lieu et scene', 'Lieu et sc&#232;ne du jeu o&#249; vont se d&#233;rouler votre jeu sont d&#233;crit ici.', 'resources/table_icons/application_view_gallery.png', 'Domaine cr&#233;atif'),
			'ref_badges' => array('Badges ref', '', 'table.gif', 'Referentiel'),
			'ref_sequences' => array('R&#233;f&#233;rentiel des s&#233;quences', 'Vous trouverez ici le r&#233;f&#233;rentiel des s&#233;quences d\'une histoire, selon la nomenclature de V.Propp ou de ...', 'table.gif', 'Referentiel'),
			'ref_mission' => array('R&#233;f&#233;rentiel des missions', 'Vous trouverez ici le r&#233;f&#233;rentiel des missions, selon la nomenclature de Quest2learn', 'table.gif', 'Referentiel'),
			'boiteaid' => array('Boite &#224; id&#233;es', 'Stocker dans cette table les id&#233;es de missions et d\'utilisation de capteurs', 'resources/table_icons/lightbulb.png', 'Zone d\'assemblage Creative'),
			'ref_capteur' => array('Ref capteur Machine', 'R&#233;f&#233;rentiel des capteurs disponibles dans AppInventor et sur le robot.', 'resources/table_icons/android.png', 'Referentiel')
		);
	...
	function get_table_groups($skip_authentication = false){
		...
		$all_groups = array('Zone d\'assemblage Creative', 'Domaine cr&#233;atif', 'Referentiel');
	...
	function get_sql_fields($table_name){
		$sql_fields = array(   
			'redacteur' => "`redacteur`.`id_redact` as 'id_redact', `redacteur`.`member_id` as 'member_id', `redacteur`.`prenom` as 'prenom', `redacteur`.`nom` as 'nom', `redacteur`.`email` as 'email', `redacteur`.`score_total` as 'score_total', IF(    CHAR_LENGTH(`ref_badges1`.`titre`) || CHAR_LENGTH(`ref_badges1`.`icone_badge`), CONCAT_WS('',   `ref_badges1`.`titre`, `ref_badges1`.`icone_badge`), '') as 'badges', `redacteur`.`score_hist` as 'score_hist', `redacteur`.`score_obj` as 'score_obj', `redacteur`.`score_moy` as 'score_moy', `redacteur`.`score_objets` as 'score_objets', `redacteur`.`score_perso` as 'score_perso', `redacteur`.`score_lets` as 'score_lets'",
			'Dossier_histoire' => "`Dossier_histoire`.`id2dossier` as 'id2dossier', IF(    CHAR_LENGTH(`redacteur1`.`prenom`) || CHAR_LENGTH(`redacteur1`.`nom`), CONCAT_WS('',   `redacteur1`.`prenom`, ' ', `redacteur1`.`nom`), '') as 'auteur', `Dossier_histoire`.`titre_histoire` as 'titre_histoire', `Dossier_histoire`.`resume` as 'resume', `Dossier_histoire`.`creer_par` as 'creer_par'",
			'chapitre' => "`chapitre`.`id` as 'id', `chapitre`.`titre` as 'titre', IF(    CHAR_LENGTH(`ref_mission1`.`titre`) || CHAR_LENGTH(`ref_mission1`.`description`), CONCAT_WS('',   `ref_mission1`.`titre`, ' : ', `ref_mission1`.`description`), '') as 'Mission', `chapitre`.`resume` as 'resume', IF(    CHAR_LENGTH(`Dossier_histoire1`.`titre_histoire`), CONCAT_WS('',   `Dossier_histoire1`.`titre_histoire`), '') as 'rattache_a_dossier', `chapitre`.`auteur` as 'auteur'",
			'sequence' => "`sequence`.`id_sequence` as 'id_sequence', `sequence`.`numero_ordre` as 'numero_ordre', `sequence`.`titre` as 'titre', IF(    CHAR_LENGTH(`chapitre1`.`titre`), CONCAT_WS('',   `chapitre1`.`titre`), '') as 'rattache_a_chapitre', `sequence`.`difficulte` as 'difficulte', `sequence`.`description` as 'description', `sequence`.`auteur` as 'auteur', IF(    CHAR_LENGTH(`ref_sequences1`.`typologie`) || CHAR_LENGTH(`ref_sequences1`.`titre`), CONCAT_WS('',   `ref_sequences1`.`typologie`, ' : ', `ref_sequences1`.`titre`), '') as 'modele'",
			'Objectifs' => "`Objectifs`.`id_objectif` as 'id_objectif', `Objectifs`.`titreobj` as 'titreobj', `Objectifs`.`description_obj` as 'description_obj', `Objectifs`.`ref_o_gameplay` as 'ref_o_gameplay', `Objectifs`.`creer_par` as 'creer_par', IF(    CHAR_LENGTH(`Dossier_histoire1`.`titre_histoire`), CONCAT_WS('',   `Dossier_histoire1`.`titre_histoire`), '') as 'rattache_a_dos2j'",
			'moyens' => "`moyens`.`id_moyen` as 'id_moyen', `moyens`.`titremoy` as 'titremoy', `moyens`.`description_moy` as 'description_moy', `moyens`.`ref_o_gameplay` as 'ref_o_gameplay', `moyens`.`creer_par` as 'creer_par', IF(    CHAR_LENGTH(`Dossier_histoire1`.`titre_histoire`), CONCAT_WS('',   `Dossier_histoire1`.`titre_histoire`), '') as 'rattache_a_dos2j'",
			'Objets' => "`Objets`.`id_objet` as 'id_objet', `Objets`.`nom_objet` as 'nom_objet', `Objets`.`illustration` as 'illustration', `Objets`.`typologie` as 'typologie', `Objets`.`Aquoisertil` as 'Aquoisertil', IF(    CHAR_LENGTH(`Objets1`.`nom_objet`) || CHAR_LENGTH(`Objets1`.`illustration`), CONCAT_WS('',   `Objets1`.`nom_objet`, `Objets1`.`illustration`), '') as 'Contient', `Objets`.`creer_par` as 'creer_par', IF(    CHAR_LENGTH(`Dossier_histoire1`.`titre_histoire`), CONCAT_WS('',   `Dossier_histoire1`.`titre_histoire`), '') as 'rattache_a_dos2j', IF(    CHAR_LENGTH(`sequence1`.`numero_ordre`) || CHAR_LENGTH(`sequence1`.`titre`), CONCAT_WS('',   `sequence1`.`numero_ordre`, ' ', `sequence1`.`titre`), '') as 'rattache_a_seq'",
			'personnages' => "`personnages`.`id_perso` as 'id_perso', `personnages`.`nom` as 'nom', `personnages`.`type` as 'type', `personnages`.`illustration` as 'illustration', `personnages`.`description` as 'description', IF(    CHAR_LENGTH(`Objets1`.`nom_objet`) || CHAR_LENGTH(`Objets1`.`illustration`), CONCAT_WS('',   `Objets1`.`nom_objet`, `Objets1`.`illustration`), '') as 'quepossedetil', `personnages`.`apparait_quand` as 'apparait_quand', `personnages`.`disparait_quand` as 'disparait_quand', `personnages`.`creer_par` as 'creer_par', IF(    CHAR_LENGTH(`Dossier_histoire1`.`titre_histoire`), CONCAT_WS('',   `Dossier_histoire1`.`titre_histoire`), '') as 'rattache_a_dos2j', IF(    CHAR_LENGTH(`sequence1`.`numero_ordre`) || CHAR_LENGTH(`sequence1`.`titre`), CONCAT_WS('',   `sequence1`.`numero_ordre`, ' ', `sequence1`.`titre`), '') as 'rattache_a_seq'",
			'lieu_et_scene' => "`lieu_et_scene`.`id_lets` as 'id_lets', `lieu_et_scene`.`nom_du_lieu` as 'nom_du_lieu', `lieu_et_scene`.`illustration` as 'illustration', `lieu_et_scene`.`description` as 'description', `lieu_et_scene`.`difficulte_nivo` as 'difficulte_nivo', `lieu_et_scene`.`condition_entree` as 'condition_entree', `lieu_et_scene`.`condition_sortie` as 'condition_sortie', IF(    CHAR_LENGTH(`Objets1`.`nom_objet`) || CHAR_LENGTH(`Objets1`.`illustration`), CONCAT_WS('',   `Objets1`.`nom_objet`, `Objets1`.`illustration`), '') as 'objet_present', IF(    CHAR_LENGTH(`personnages1`.`nom`) || CHAR_LENGTH(`personnages1`.`illustration`), CONCAT_WS('',   `personnages1`.`nom`, `personnages1`.`illustration`), '') as 'persos_present', `lieu_et_scene`.`creer_par` as 'creer_par', IF(    CHAR_LENGTH(`Dossier_histoire1`.`titre_histoire`), CONCAT_WS('',   `Dossier_histoire1`.`titre_histoire`), '') as 'rattache_a_dos2j', IF(    CHAR_LENGTH(`sequence1`.`numero_ordre`) || CHAR_LENGTH(`sequence1`.`titre`), CONCAT_WS('',   `sequence1`.`numero_ordre`, ' ', `sequence1`.`titre`), '') as 'rattache_a_seq'",
			'ref_badges' => "`ref_badges`.`id_badge` as 'id_badge', `ref_badges`.`titre` as 'titre', `ref_badges`.`description` as 'description', `ref_badges`.`domaine` as 'domaine', `ref_badges`.`score_min` as 'score_min', `ref_badges`.`score_max` as 'score_max', `ref_badges`.`icone_badge` as 'icone_badge'",
			'ref_sequences' => "`ref_sequences`.`id_seqref` as 'id_seqref', `ref_sequences`.`titre` as 'titre', `ref_sequences`.`description` as 'description', `ref_sequences`.`typologie` as 'typologie', `ref_sequences`.`num_ord` as 'num_ord'",
			'ref_mission' => "`ref_mission`.`id_misref` as 'id_misref', `ref_mission`.`titre` as 'titre', `ref_mission`.`description` as 'description'",
			'boiteaid' => "`boiteaid`.`id` as 'id', `boiteaid`.`titre` as 'titre', `boiteaid`.`commentaire` as 'commentaire', `boiteaid`.`Capteur` as 'Capteur'",
			'ref_capteur' => "`ref_capteur`.`id` as 'id', `ref_capteur`.`nom` as 'nom', `ref_capteur`.`fonction` as 'fonction', `ref_capteur`.`machine` as 'machine', `ref_capteur`.`reference` as 'reference', `ref_capteur`.`fichier` as 'fichier'"
		);
	...
	function get_sql_from($table_name, $skip_permissions = false){
		$sql_from = array(   
			'redacteur' => "`redacteur` LEFT JOIN `ref_badges` as ref_badges1 ON `ref_badges1`.`id_badge`=`redacteur`.`badges` ",
			'Dossier_histoire' => "`Dossier_histoire` LEFT JOIN `redacteur` as redacteur1 ON `redacteur1`.`id_redact`=`Dossier_histoire`.`auteur` ",
			'chapitre' => "`chapitre` LEFT JOIN `ref_mission` as ref_mission1 ON `ref_mission1`.`id_misref`=`chapitre`.`Mission` LEFT JOIN `Dossier_histoire` as Dossier_histoire1 ON `Dossier_histoire1`.`id2dossier`=`chapitre`.`rattache_a_dossier` ",
			'sequence' => "`sequence` LEFT JOIN `chapitre` as chapitre1 ON `chapitre1`.`id`=`sequence`.`rattache_a_chapitre` LEFT JOIN `ref_sequences` as ref_sequences1 ON `ref_sequences1`.`id_seqref`=`sequence`.`modele` ",
			'Objectifs' => "`Objectifs` LEFT JOIN `Dossier_histoire` as Dossier_histoire1 ON `Dossier_histoire1`.`id2dossier`=`Objectifs`.`rattache_a_dos2j` ",
			'moyens' => "`moyens` LEFT JOIN `Dossier_histoire` as Dossier_histoire1 ON `Dossier_histoire1`.`id2dossier`=`moyens`.`rattache_a_dos2j` ",
			'Objets' => "`Objets` LEFT JOIN `Objets` as Objets1 ON `Objets1`.`id_objet`=`Objets`.`Contient` LEFT JOIN `Dossier_histoire` as Dossier_histoire1 ON `Dossier_histoire1`.`id2dossier`=`Objets`.`rattache_a_dos2j` LEFT JOIN `sequence` as sequence1 ON `sequence1`.`id_sequence`=`Objets`.`rattache_a_seq` ",
			'personnages' => "`personnages` LEFT JOIN `Objets` as Objets1 ON `Objets1`.`id_objet`=`personnages`.`quepossedetil` LEFT JOIN `Dossier_histoire` as Dossier_histoire1 ON `Dossier_histoire1`.`id2dossier`=`personnages`.`rattache_a_dos2j` LEFT JOIN `sequence` as sequence1 ON `sequence1`.`id_sequence`=`personnages`.`rattache_a_seq` ",
			'lieu_et_scene' => "`lieu_et_scene` LEFT JOIN `Objets` as Objets1 ON `Objets1`.`id_objet`=`lieu_et_scene`.`objet_present` LEFT JOIN `personnages` as personnages1 ON `personnages1`.`id_perso`=`lieu_et_scene`.`persos_present` LEFT JOIN `Dossier_histoire` as Dossier_histoire1 ON `Dossier_histoire1`.`id2dossier`=`lieu_et_scene`.`rattache_a_dos2j` LEFT JOIN `sequence` as sequence1 ON `sequence1`.`id_sequence`=`lieu_et_scene`.`rattache_a_seq` ",
			'ref_badges' => "`ref_badges` ",
			'ref_sequences' => "`ref_sequences` ",
			'ref_mission' => "`ref_mission` ",
			'boiteaid' => "`boiteaid` ",
			'ref_capteur' => "`ref_capteur` "
		);

		$pkey = array(   
			'redacteur' => 'id_redact',
			'Dossier_histoire' => 'id2dossier',
			'chapitre' => 'id',
			'sequence' => 'id_sequence',
			'Objectifs' => 'id_objectif',
			'moyens' => 'id_moyen',
			'Objets' => 'id_objet',
			'personnages' => 'id_perso',
			'lieu_et_scene' => 'id_lets',
			'ref_badges' => 'id_badge',
			'ref_sequences' => 'id_seqref',
			'ref_mission' => 'id_misref',
			'boiteaid' => 'id',
			'ref_capteur' => 'id'
		);
	...
	function get_defaults($table){
		...
		$defaults = array(
			...
			'chapitre' => array(
				...
				'titre' => '',
				'Mission' => '',
				'resume' => 'L\'histoire est la suivante...',
				'rattache_a_dossier' => '',
				'auteur' => ''
			),
			'sequence' => array(
			...
			'Objectifs' => array(
				'id_objectif' => '',
				'titreobj' => '',
				'description_obj' => 'Le joueur doit ...',
				'ref_o_gameplay' => '',
				'creer_par' => '',
				'rattache_a_dos2j' => ''
			),
			'moyens' => array(
				'id_moyen' => '',
				'titremoy' => '',
				'description_moy' => 'Le joueur peut ...',
				'ref_o_gameplay' => '',
				'creer_par' => '',
				'rattache_a_dos2j' => ''
			),
			...
			'personnages' => array(
				...
				'description' => '',
				'quepossedetil' => '',
				'apparait_quand' => 'Il apparait quand...',
				'disparait_quand' => 'Il disparait quand...',
				'creer_par' => '',
				'rattache_a_dos2j' => '',
				'rattache_a_seq' => ''
			),
			'lieu_et_scene' => array(
				...
				'description' => '',
				'difficulte_nivo' => '',
				'condition_entree' => 'Pour acc&#233;der &#224; cette sc&#232;ne...',
				'condition_sortie' => 'Pour sortir de cette sc&#232;ne...',
				'objet_present' => '',
				'persos_present' => '',
				'creer_par' => '',
				'rattache_a_dos2j' => '',
	...
	function logInMember(){
		...
		if($_POST['signIn'] != ''){
			if($_POST['username'] != '' && $_POST['password'] != ''){
				...
				if(sqlValue("select count(1) from membership_users where lcase(memberID)='$username' and passMD5='$password' and isApproved=1 and isBanned=0")==1){
					...
					if($_POST['rememberMe']==1){
						@setcookie('Voyajhist_rememberMe', md5($username.$password), time()+86400*30);
					}else{
						@setcookie('Voyajhist_rememberMe', '', time()-86400*30);
					}
			...
			exit;
		}elseif((!$_SESSION['memberID'] || $_SESSION['memberID']==$adminConfig['anonymousMember']) && $_COOKIE['Voyajhist_rememberMe']!=''){
			$chk=makeSafe($_COOKIE['Voyajhist_rememberMe']);
	...
	function htmlUserBar(){
		...
		<nav class="navbar navbar-default navbar-fixed-top hidden-print" role="navigation">
			<div class="navbar-header">
				...
				<a class="navbar-brand" href="<?php echo PREPEND_PATH; ?>index.php"><i class="glyphicon glyphicon-home"></i> Voyajhist</a>
			</div>
