...
function Objectifs_insert(){
	...
	set_record_owner('Objectifs', $recID, getLoggedMemberID());
...
function Objectifs_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = ''){
	...
	// process values
	if($selected_id){
		if( $dvprint) $templateCode = str_replace('<%%VALUE(id_objectif)%%>', safe_html($urow['id_objectif']), $templateCode);
		if(!$dvprint) $templateCode = str_replace('<%%VALUE(id_objectif)%%>', html_attr($row['id_objectif']), $templateCode);
		$templateCode = str_replace('<%%URLVALUE(id_objectif)%%>', urlencode($urow['id_objectif']), $templateCode);
		if( $dvprint) $templateCode = str_replace('<%%VALUE(titreobj)%%>', safe_html($urow['titreobj']), $templateCode);
		if(!$dvprint) $templateCode = str_replace('<%%VALUE(titreobj)%%>', html_attr($row['titreobj']), $templateCode);
	...
	if($selected_id){
		...
		if($jdata === false) $jdata = get_defaults('Objectifs');
		$rdata = $row;
	}
	$templateCode .= loadView('Objectifs-ajax-cache', array('rdata' => $rdata, 'jdata' => $jdata));
